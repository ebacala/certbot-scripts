...
if [[ $# -ne 3 ]]; then
  echo 'You need 4 arguments : your domain name (please include your subdomain name), your email adress and the port that needs to be forwarded';
  exit;
fi
...
envsubst '${NGINX_DOMAIN_NAME} ${NGINX_SUB_DOMAIN_NAME} ${NGINX_APP_PORT}' < app.template > /etc/nginx/sites-available/${NGINX_DOMAIN_NAME}
